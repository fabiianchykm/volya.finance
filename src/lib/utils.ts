...
export function formatCompanyName(name: string): string {
  ...
  // NB: у JS `\b` працює лише з латиницею, тож для кириличних маркерів («програма»,
  // «онлайн») НЕ використовуємо \b — інакше «ТАС програма 2» лишалось «програма 2».
  let formatted = name
    .replace(/(програма|program)\s*№?\s*\d*/gi, ' ')
    .replace(/(офлайн|онлайн|offline|online)/gi, ' ')
    .replace(/["«»„“”‘’‚'`,]/g, ' ');
