            ...
            <form onSubmit={submit} className="rounded-2xl bg-white dark:bg-zinc-900 p-5 text-left shadow-2xl sm:p-7">
              ...
              <label className="mt-5 flex cursor-pointer items-center justify-between gap-3 rounded-xl border border-zinc-200 dark:border-zinc-700 px-4 py-3 transition-colors hover:border-indigo-200">
                <span className="flex flex-col">
                  ...
                  <span className="text-xs text-zinc-400 dark:text-zinc-500">{t({ uk: "Багато поїздок за одним полісом", en: "Many trips on one policy" })}</span>
                </span>
          ...
          {cards.map((o, i) => (
            <OfferCard
              ...
              {...(() => {
                ...
                // Точний термін в інших СК не підтверджено — кажемо лише, що це НЕ рік.
                return {
                  faceNote: t({ uk: "Договір не діє 365 днів", en: "Policy is not valid for 365 days" }),
                  faceNoteMuted: true,
                  faceNoteHint: t({
                    uk: "У цієї страхової договір мультивізи діє не цілий рік. Точний термін дії буде вказано в полісі.",
                    en: "This insurer's multi-trip policy is not valid for a full year. The exact term is stated in the policy.",
                  }),
                };
              })()}
