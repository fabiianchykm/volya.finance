  ...
  try {
    ...
    if (SUPPORT_GROUP_ID) {
      ...
      if (chatId === String(SUPPORT_GROUP_ID)) {
        ...
        // Клієнту показуємо лише текст відповіді — без імені оператора (єдиний
        // «голос» служби турботи, щоб не було різнобою з привітанням Володимира).
        if (text) {
          await tg("sendMessage", { chat_id: clientId, parse_mode: "HTML", text: esc(text) });
        } else {
    ...
    if (isOperator) {
      ...
      if (m && text) {
        await tg("sendMessage", { chat_id: m[1], parse_mode: "HTML", text: esc(text) });
